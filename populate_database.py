...
aerts_country_list = ['fr', 'ie', 'nl', 'co.uk', 'ch', 'lu', 'be', 'es', 'it', 'tr', 'se', 'pt',
                      'mt', 'fi', 'si', 'ro', 'de', 'lt', 'lv', 'gr', 'cz', 'hu', 'hr', 'sk', 'ee', 'no', 'bg', 'pl']
...
cursor = conn.cursor()
cursor.execute(
    "CREATE TABLE if not exists visits (visit_id bigint, site_nr int, sitename varchar(255), "
    "visit_type int, site_url varchar(255), cookie_numbers int)")
cursor.execute(
    "CREATE TABLE if not exists cookies (visit_id bigint, before_after varchar(24), short_url varchar(255), "
    "domain varchar(255), expires float(24), httpOnly bool, name varchar(255), path varchar(255), priority varchar(24), "
    "sameParty bool, sameSite varchar(25), secure bool, session bool, size int, sourcePort int, sourceScheme varchar(255), value varchar(255))")
cursor.execute(
    "CREATE TABLE if not exists elements (visit_id bigint, site_nr int, sitename varchar(255), element_type tinyint, "
    "visited tinyint, result varchar(255), element_text varchar(1024), element_css varchar(255), iframe_css varchar(255), "
    "location_x int, location_y int, text_color varchar(255), background_color varchar(255), width varchar(24), height varchar(24), font_size varchar(24), PRIMARY KEY (site_nr, element_type))")
cursor.execute(
    "CREATE TABLE if not exists redirects (visit_id bigint, status_code int, date varchar(32), url_from varchar(1024), "
    "url_to varchar(1024), content_type varchar(264))")
cursor.execute(
    "CREATE TABLE if not exists all_requests (visit_id bigint, status_code int, date varchar(32), "
    "url_from varchar(1024), url_to varchar(1024), content_type varchar(264))")
cursor.execute(
    "CREATE TABLE if not exists predictions (visit_id bigint, element_type varchar(32), element_type2 varchar(32), "
    "element_text varchar(1024), prediction varchar(32))")
